...
def train_two_stage_rl(env, perception_agent, reasoning_agent, 
                      optimizer, num_episodes, batch_size, beta=0.5):
    experience_buffer = ExperienceBuffer()
    vis_manager = VisualizationManager(env.symptom_set, env.disease_set)
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            disease_probs = reasoning_agent(masked_symptoms.unsqueeze(0))  # Add batch dimension
            predicted_disease = torch.argmax(disease_probs).item()
            
            # Environment step
            next_state, reward, done, info = env.step(predicted_disease)

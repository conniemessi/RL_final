...
# Convert binary values to symptom lists
def binary_to_symptoms(row):
    present_symptoms = []
    for i, value in enumerate(row[:-1]):  # Exclude the last column (diagnosis)
        if value == 1:
            present_symptoms.append(symptom_names[i])
    return str(present_symptoms)

# Create new dataframe in desired format
new_df = pd.DataFrame({
    'symptoms': df.iloc[:, :-2].apply(binary_to_symptoms, axis=1),
    'diagnosis': df.iloc[:, -2]
})

# Save to CSV
new_df.to_csv('converted_training.csv', index=False)

# Print first few rows to verify the format
print(new_df.head())